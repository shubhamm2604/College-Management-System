...
class Student extends Component {
  ...
  constructor(props) {
    ...
    this.UpdateStud = this.UpdateStud.bind(this)
  }
...
SearchId(){
  this.setState({SearchId: true});
  console.log(this.state);
}


SearchStud(){
  this.setState({SearchStud: true});
  console.log(this.state);
}

UpdateStud(){
  this.setState({UpdateStud: true});
  console.log(this.state);
}
...
handleSubmit(event) {
  ...
  render() {
    ...
    if(this.state.SearchId){
      return (<Redirect to={'/searchId'} />)
    }
    
    if(this.state.SearchStud){
      return (<Redirect to={'/searchstud'} />)
    }

   
    if(this.state.UpdateStud){
    ...
    <div  className="form">
      <form id="contactform" method='POST' action='/' onSubmit={this.handleSubmit}>
        ...
        <input id="phone" name="Id" placeholder="Enrollment no" required="" type="text"  onChange={this.onChange} /> <br/>
        ...
        <label className="month">
                <select className="select-style" name="Course" onChange={this.onChange} >
        ...
        <input className="stdbtn1" name="search" id="submit1" value="Search All" type="submit" onClick={this.SearchStud}  />  <br/> 
        <input className="stdbtn2" name="update" id="submit2" value="Update/Delete" type="submit" onClick={this.UpdateStud} />  <br/>
        <input className="stdbtn3" name="delete" id="submit3" value="Search" type="submit" onClick={this.SearchId} />  <br/>
